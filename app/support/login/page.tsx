...
export default function SupportLoginPage() {
  ...
  return <main className="support-auth-page"><div className="support-auth-mark"><span>沐</span><small>MUCHEN SUPPORT</small></div><section className="support-auth-card"><span className="support-eyebrow">CUSTOMER SERVICE CONSOLE</span><h1>客服工作台</h1><p>管理试用邀请，为用户发放进入沐尘研究工作区的访问凭证。</p><form onSubmit={submit}><label>客服邮箱<input type="email" placeholder="support@example.com" value={email} onChange={(event) => setEmail(event.target.value)} required /></label><label>客服访问密钥<input type="password" placeholder="请输入客服访问密钥" value={accessKey} onChange={(event) => setAccessKey(event.target.value)} required /></label><button className="support-primary" type="submit" disabled={loading}>{loading ? "验证中…" : "进入客服工作台"}<span>→</span></button></form>{error && <p className="support-error">{error}</p>}<small className="support-auth-note">仅限沐尘客服人员使用 · 生产环境必须配置独立访问密钥</small><small className="support-demo-note">本地演示：客服邮箱任意有效邮箱，访问密钥 <code>MUCHEN-SUPPORT-DEMO</code></small></section></main>;
}
