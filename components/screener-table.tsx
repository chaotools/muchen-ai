...
export default function ScreenerTable({ universe }: { universe: ScreenerQuote[] }) {
  ...
  const filtered = useMemo(() => universe.filter((quote) => {
    ...
    return industryMatch && momentumMatch && (minScore === 0 || (quote.score !== null && quote.score >= minScore));
  ...
  return (
    <>
      <div className="screener-toolbar">
        ...
        <div className="screener-filter-group"><span>演示评分</span>{scoreOptions.map((score) => <button className={minScore === score ? "filter-chip active" : "filter-chip"} key={score} onClick={() => setMinScore(score)}>{score === 0 ? "全部" : `${score}+`}</button>)}</div>
      </div>
      ...
      <section className="market-card screener-card">
        ...
        <div className="screener-table-wrap">
          <div className="screener-row screener-head"><span>标的</span><span>最新价</span><span>涨跌幅</span><span>演示评分</span><span>动能</span><span>估值</span><span>风险</span></div>
          {filtered.map((quote) => <Link className="screener-row" href={`/stocks/${quote.code}`} key={quote.code}><span className="stock-cell"><strong>{quote.name}</strong><small>{quote.code} · {quote.industry} · {quote.asOf ?? "演示样本"}</small></span><strong>¥{quote.price.toFixed(2)}</strong><span className={quote.changePercent >= 0 ? "positive" : "negative"}>{quote.changePercent >= 0 ? "+" : ""}{quote.changePercent.toFixed(2)}%</span><strong className={(quote.score ?? 0) >= 80 ? "positive" : "score-normal"}>{quote.score ?? "未评估"}</strong><span className={`table-tag momentum-${quote.momentum}`}>{quote.momentum}</span><span className="table-tag">{quote.valuation}</span><span className={`table-tag risk-${quote.risk}`}>{quote.risk}</span></Link>)}
