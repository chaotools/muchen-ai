...
const ranges: Record<ChartRange, { label: string; timestamps: string[]; changes: number[] }> = {
  intraday: {
    label: "分时",
...
export default function StockChart({ code, price, high52, low52 }: StockChartProps) {
  const [range, setRange] = useState<ChartRange>("intraday");
  const [hoverIndex, setHoverIndex] = useState<number | null>(null);
  const series = ranges[range];
  const points = series.changes.map((change) => price * (1 + change / 100));
    ...
    <>
      ...
      <div className="chart-area interactive-chart">
        ...
        <div className="interactive-chart-canvas">
          <svg viewBox="0 0 760 240" className="chart-svg" role="img" aria-label={`${series.label}演示走势，可移动指针查看数值`} onPointerMove={updateHover} onPointerLeave={() => setHoverIndex(null)}>
      ...
      <div className="chart-legend"><span><i className="legend-line" />{series.label}演示走势</span><span>52 周高 ¥{high52.toFixed(2)}</span><span>52 周低 ¥{low52.toFixed(2)}</span></div>
