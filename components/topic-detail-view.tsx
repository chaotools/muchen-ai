  ...
  return (
    <>
      ...
      <section className="topic-detail-kpis">
        <div><span>综合热度</span><strong className="topic-heat-value">{topic.heat}</strong><small>较上一样本 {topic.heat - previousHeat >= 0 ? "+" : ""}{topic.heat - previousHeat} 点</small></div>
      ...
      <div className="topic-detail-content-grid topic-detail-bottom-grid">
        ...
        <section className="market-card topic-events-detail-card"><div className="topic-card-heading"><div><span className="eyebrow">EVENT TIMELINE</span><h2>催化与动态</h2></div><span className="muted">{topic.events.length} 条记录</span></div><div className="topic-detail-events">{topic.events.map((event) => <div className="topic-detail-event" key={event.id}><span className={`topic-event-dot topic-event-${event.tone}`} /><div><div className="topic-event-meta"><span>{event.date} {event.time}</span><em>{event.type}</em></div><strong>{event.title}</strong><p>{event.summary}</p></div></div>)}</div><div className="topic-source-note">数据状态：{topic.dataStatus === "free-data" ? `本地免费数据 · 更新于 ${topic.asOf ?? "最新交易日"}` : "演示数据"}</div></section>
      </div>
